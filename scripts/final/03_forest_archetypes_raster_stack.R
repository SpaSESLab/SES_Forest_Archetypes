...
millchng_crop <-  rast(here::here("data/processed/variables/millchange_interp-2_2024-10-04.tif"))
...
writeRaster(x = rast_stack, 
            filename = paste0("/Users/katiemurenbeeld/Analysis/Archetype_Analysis/data/processed/rast_stack_all_attributes_", 
                                              Sys.Date(), ".tif"), 
            overwrite = TRUE)

writeRaster(x = rast_stack_sc, 
            filename = paste0("/Users/katiemurenbeeld/Analysis/Archetype_Analysis/data/processed/rast_stack_all_attributes_scaled_", 
                                              Sys.Date(), ".tif"), 
            overwrite = TRUE)
...
writeRaster(x = soc_rast_stack, 
            filename = here::here(paste0("data/processed/raster_stacks/rast_stack_soc_attributes_", 
                                         Sys.Date(), ".tif")), 
            overwrite = TRUE)

writeRaster(x = soc_rast_stack_sc, 
            filename = here::here(paste0("data/processed/raster_stacks/rast_stack_soc_attributes_scaled_", 
                                         Sys.Date(), ".tif")), 
            overwrite = TRUE)
